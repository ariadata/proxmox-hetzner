...
configure_proxmox_via_ssh() {
    ...
    for i in {1..60}; do
        if sshpass -p "$NEW_ROOT_PASSWORD" ssh $SSH_OPTS root@localhost 'true' 2>/dev/null; then
            echo -e "${CLR_GREEN}SSH login OK.${CLR_RESET}"
            break
        fi
        echo -n "."
        sleep 5
        if [ "$i" -eq 60 ]; then
            echo -e "${CLR_RED}SSH login failed after waiting.${CLR_RESET}"
            return 1
        fi
    done
